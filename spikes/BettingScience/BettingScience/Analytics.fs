...
let check fileName =
    ...
    let computeHomeAway score odd =
        ...
        | [|{ Value = None;
            ...
            printfn "Score (%A:%A) Starting %A Closing %A" score.Home score.Away s1 c1
            ...
            startingSum <- startingSum + (getMoney Home s1)
            closingSum <- closingSum + (getMoney Home s1)
    ...
    for m in matches do
        let oddsValue = m.Odds |> Array.tryFind (fun out -> out.OutcomeID = OddsPortalScraper.outAsianHandicapID)
        oddsValue |>> computeHandicap m.Score |> ignore